...
class BoxHandler:
    ...
    def __init__(self):
        ...
        self.orientation = [0.0, 0.0, 0.0]  # r, p, s
        self.sign = 1
    ...
    def do_spawn_model(self):
        ...
        try:
            ...
            rospy.logerr(
                "%s not found. export GAZEBO_MODEL_PATH=models_dir_path",
                BoxHandler.MODEL_FILE,
            )
            sys.exit(-1)
        ...
        model_pose.orientation = Quaternion(
            *transform.quaternion_from_euler(0, 0, 0)
        )  # roll, pitch, yaw
    ...
    def update_model(self):
        ...
        self.position[2] = z
        if (
            x < BoxHandler.INIT_POSE[0]
            or x > BoxHandler.INIT_POSE[0] + BoxHandler.XMAX_BOX
        ):
...
if __name__ == "__main__":
    ...
    rospy.loginfo(
        "The following shell command should be run: export ROBOT_INITIAL_POSE='-Y 1.57'"
    )
